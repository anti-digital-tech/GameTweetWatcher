...
const CELL_SETTINGS_TITLE_BAN_USERS         = "[Ban Users]";
const CELL_SETTINGS_TITLE_FILTER_WORDS      = "[Filter Words]";
const CELL_SETTINGS_TITLE_ALERT_THRESHOLD_FP = "[Alert Threshold Filtered Percentage]";
const CELL_SETTINGS_TITLE_DOWNLOAD_MEDIA    = "[Download Media]";
...
const CELL_SETTINGS_TITLE_ALERT_THRESHOLD   = "[Alert Threshold Points]";

const CELL_SETTINGS_TITLE_LAST_KEYWORD      = "[[Last Keyword]]";
const CELL_SETTINGS_TITLE_LAST_UPDATED      = "[[Last Updated]]";
const CELL_SETTINGS_TITLE_END_SETTINGS      = "//end of settings";

const CELL_HEADER_TITLES_DATA               = {tweetId:"Tweet Id", createdAt:"Created at", userId:"User Id", userName:"User name", screenName:"Screen name", retweet:"Retweet", favorite:"Favorite", tweet:"Tweet", media:"Media"};
const CELL_HEADER_TITLES_HISTORY            = {dateTime:"date time", keyword:"keyword", ptTweet:"pt/Tweet", ptRetweet:"pt/Retweet", ptMedia:"pt/Media", ptFav:"pt/Fav", threshold:"Threshold", blank:"", total:"Total", tweets:"Tweets", retweets:"Retweets", media:"Media", favs:"Favs"};
...
const DEFAULT_ALERT_THRESHOLD_RWP   = 30;
...
function sendMail(sheet, stats:Stats, settings:Settings) {
  ...
  let body = "Total Points : " + stats.points + ", " + "Perentage of Filtered Words: " + stats.percentWords;
...
function inseartHeaderData(sheet, row) {
  let rangeHeader = sheet.getRange(row, 1, 1, Object.keys(CELL_HEADER_TITLES_DATA).length);
  let valsHeader = rangeHeader.getValues();
  Object.keys(CELL_HEADER_TITLES_DATA).forEach( (val, c) =>{
    valsHeader[0][c] = val;
  } );
...
function inseartHeaderHistory(sheet, row) {
  let rangeHeader = sheet.getRange(row, 1, 1, Object.keys(CELL_HEADER_TITLES_HISTORY).length);
  let valsHeader = rangeHeader.getValues();
  Object.keys(CELL_HEADER_TITLES_HISTORY).forEach( (val, c) =>{
    valsHeader[0][c] = val;
  } );
  rangeHeader.setValues(valsHeader);
}
...
function addHistory( nameSheet:string, indexSheet:number, stats:Stats, settings:Settings ) {
  ...
  let rangeLastRow = sheetHistory.getRange( rowToWrite, 1, 1, Object.keys(CELL_HEADER_TITLES_HISTORY).length);
  if (rangeLastRow) {
...
function addTweetDataAtBottom(sheet, indexSheet:number, tweet:Tweet, settings:Settings, stats:Stats ) {
  try {
    ...
    let rangeLastRow = sheet.getRange( rowToWrite, 1, 1, Object.keys(CELL_HEADER_TITLES_DATA).length);
    if (rangeLastRow) {
      let valsRange = rangeLastRow.getValues();
      let row = valsRange[0];
...
//
function hasWords ( tweet:Tweet, words:string[] ){
...
//
function isValidTweet( tweet:Tweet, settings:Settings ) {
  let keywords = settings.currentKeyword.split(/\s/);
...
function isNeededToBeAdded( sheet, tweet:Tweet, settings:Settings) {
  ...
  if ( 0 < lastRow - (settings.headerInfo.idx_row + 1 )) {
    let range = sheet.getRange(settings.headerInfo.idx_row + 2, 1, lastRow - (settings.headerInfo.idx_row + 1), Object.keys(CELL_HEADER_TITLES_DATA).length);
    if (range) {
      let valsRange = range.getValues();
...
function getSettingsActual( settingsCommon:Settings, settingsLocal:Settings ) {
  return new Settings (
    ...
    , settingsCommon.filterWords.concat(settingsLocal.filterWords)
    , settingsLocal.fpAlertThreshold ? settingsLocal.fpAlertThreshold : ( settingsCommon.fpAlertThreshold ? settingsCommon.fpAlertThreshold : DEFAULT_ALERT_THRESHOLD_RWP )
...
function getHeaderInfo( sheet ) {
  let range = sheet.getRange( 1, 1, MAX_ROW_SEEK_HEADER, Object.keys(CELL_HEADER_TITLES_DATA).length);
  if (range == null) {
    return;
  }
  let valsRange = range.getValues();
...
function moveData( nameBookSrc:string, fullYear:number, month:number, nameSheet:string, indexSheet:number, valsRangeSrc:number[][], rowStart:number, rowNum:number ) {
  try {
    ...
    let rangeBackup = sheetBackup.getRange(sheetBackup.getLastRow() + 1, 1, rowNum, Object.keys(CELL_HEADER_TITLES_DATA).length);
    if ( rangeBackup ) {
      let valsRangeBackup = rangeBackup.getValues();
      for ( let r=0; r<rowNum; r++) {
        for (let c=0; c< Object.keys(CELL_HEADER_TITLES_DATA).length; c++){
...
function backup() {
  ...
  sheets.forEach((sheet) => {
    ...
    if ( 0 < lastRow - (headerInfo.idx_row + 1 )) {
      let range = sheet.getRange(headerInfo.idx_row + 2, 1, lastRow - (headerInfo.idx_row + 1), Object.keys(CELL_HEADER_TITLES_DATA).length);
      if (range) {
        let valsRange = range.getValues();
